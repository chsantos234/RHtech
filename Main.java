...
class Empresa{
	...
	void ConsultarLista(int Num,String Input) {
		...
		if(Num == 1) {
			for(Funcionario i : Contrato.listaFuncionarios) {
				if(i.getNome().equals(Input)) {
					...
					System.out.printf("Nome:%s,%s,Endereço:%s,%s,%s,%s,%s,CPF:%s,Telefone:%s,Idade:%d.%n",i.getNome(),i.getSobrenome(),i.getEndereco().getBairro(),i.getEndereco().getCidade(),i.getEndereco().getEstado(),i.getSexo(),i.getEmail(),i.getCpf(),i.getTelefone(),i.getIdade());
					Status = true;
						
					if(Status == true) {
						System.out.printf("ID:%d,%s,%s,Salário(R$):%.2f%n",i.getContrato().getId(),i.getContrato().getSetor().getSetorString(),i.getContrato().getCargo(),i.getContrato().getSalario());
					}
				}
	...
	// função PrintList (imprime a lista de todos os funcionários com algumas de suas informações pessoais)
	void PrintList() {
		for(Funcionario i : Contrato.listaFuncionarios) {
			System.out.printf("%s,%s,%s,%s,%s,%d%n",i.getNome(),i.getSobrenome(),i.getSexo(),i.getEmail(),i.getTelefone(),i.getIdade());
		}
	}
	...
	void AddContrato(Contrato contrato) {Contrato.listaContratos.add(contrato);}
	
	// função DemitirFuncinario (demite um funcionário específico pelo seu cpf)
	void DemitirFuncinario(String cpf) {
		boolean Status = false;
		...
		if(Status == true) {
			Contrato.listaFuncionarios.remove(remover);
			Contrato.listaContratos.remove(removerContrato);
	...
	void ProcurarSalario(int escolha) {
		...
		if(escolha == 1) {
			...
			System.out.printf("Funcionário %s %s tem o maior salário de: %.2f e trabalha no setor %s como %s.%n",funcionario.getNome(),funcionario.getSobrenome(),funcionario.getContrato().getSalario(),funcionario.getContrato().getSetor().getSetorString(),funcionario.getContrato().getCargo());
		}
		else if(escolha == 2) {
			for(Funcionario i : Contrato.listaFuncionarios) {
			...
			System.out.printf("Funcionário %s %s tem o menor salário de: %.2f e trabalha no setor %s como %s.%n",funcionario.getNome(),funcionario.getSobrenome(),funcionario.getContrato().getSalario(),funcionario.getContrato().getSetor().getSetorString(),funcionario.getContrato().getCargo());
		}
	}
	...
	static void GastoSetor() {
		...
		if(iteration4 == GastoRequisitos) {
			...
			System.out.printf("O setor de Testes tem o maior gasto de  %.2f em %d funcionário(s).%n",GastoTestes,Setor.TESTES.getFuncionarios());
		}
		else if(iteration4 == GastoRecursos) {
			System.out.printf("O setor de Recursos_humanos tem o maior gasto de  %.2f em %d funcionário(s).%n",GastoRecursos,Setor.RECURSOSHUMANOS.getFuncionarios());
		}
	}
	
	// função ProcurarSetor (retorna o setor com maior ou menor número de funcionários)
	void ProcurarSetor(int escolha) {
	...
	void AdvancedPrintList(int escolha,String setor,String sexo,String estado) {
		if(escolha == 1) {
			for(Funcionario i : Contrato.listaFuncionarios) {
...
public class Main{
	public static void main(String args[]) throws Exception{
		Endereco EnderecoEmpresa = new Endereco("Batista_Campos","Belém","PA");
		Empresa empresa = new Empresa("Manga Byte",EnderecoEmpresa,"973526628","245.925.435/2064-81","empresa@email.com","Juliano Costa");
		
		Scanner sc1 = new Scanner(System.in);
		// apresentação das ações que um usuáro pode fazer:
		System.out.printf("O software foi desenvolvido pela empresa %s. CNPJ: %s.%n",empresa.getNome(),empresa.getCnpj());
		...
		System.out.println("Visualizar funcionários por especificidade--[6]\nIdentificar funcinário por salário----------[7]\nIdentificar setor por funcionários----------[8]\nVisualizar gastos por setor-----------------[9]\nSair----------------------------------------[10]"
				+ "\n(digite qualquer outra tecla para mostrar esta tabela novamente)");
		...
		while(true) {
			...
			if(UserInput.equals("1")) {
				...
				System.out.println("Diga o sexo do funcionário: masculino[1],feminino[2],não_binário[3]:");
				...
				Setor setor = null;
				System.out.println("Escolha o setor de atuação: Requisitos[1],Modelagens[2],Codificação[3],Testes[4],Recursos_Humanos[5]:");
				...
				System.out.println("Escolha o cargo do funcionário: colaborador[1],chefe[2]:");
			...
			// (2) Consulta um funcionário específico pelo seu nome ou CPF:
				...
				System.out.println("Consulta funcionário por: nome[1],CPF[2]:");
				int ConsultaInput = sc1.nextInt();
				
				if(ConsultaInput == 1) {
					System.out.println("Digite o nome do funcionário:");
					String ConsultaNome = sc1.next();
					empresa.ConsultarLista(1,ConsultaNome);
				}
				else if(ConsultaInput == 2) {
					System.out.println("Digite o CPF do funcionário:");
					String ConsultaCPF = sc1.next();
					empresa.ConsultarLista(2,ConsultaCPF);
				}
			}
			...
			// (3) Atualizar informações pessoais ou contratuais de um funcionário específico por meio de seu nome:
				...
				System.out.println("Digite o nome do funcionário para ser atualizado:");
				...
				if(Status == true) {
					while(true) {
						...
						System.out.println("Atualizar: dados pessoais[1],dados contratuais[2],voltar[3]:");
						int UptadeInfoType = sc1.nextInt();
						Setor setor = null;
						
					// Atualizar dados pessoais
						if(UptadeInfoType == 1) {
							System.out.println("Atualizar:nome[1],sobrenome[2],endereço[3],sexo[4],e-mail[5],CPF[6],telefone[7],idade[8]:");
							...
							if(UpdatePessoal == 1) {
								...
								System.out.println("Digite um novo último nome: ");
								String NovoSobrenome = sc1.next();
								UpdatedFuncionario.setSobrenome(NovoSobrenome);
							}
							else if(UpdatePessoal == 3) {		
								System.out.println("Digite o nome do bairro (use _ como espaço): ");
								String NovoBairro = sc1.next();
								
								System.out.println("Digite o nome da cidade (use _ como espaço): ");
								String NovaCidade = sc1.next();
								
								
								System.out.println("Digite as siglas do estado");
								...
								System.out.println("Atualize o sexo: masculino[1],feminino[2],não_binário[3]:");
					...
					// Atualizar dados contratuais
							...
							System.out.println("Atualizar:setor[1],cargo[2],sálario[3]: ");
							...
							if(UpdateContratual == 1) {
								...
								System.out.println("Atualize o setor: Requisitos[1],Modelagens[2],Codificação[3],Testes[4],Recursos_Humanos[5]: ");
								...
								if(UpdatedFuncionario.getContrato().getCargo().equals("chefe")) {
									...
									setor.setChefe(0);
									System.out.println("Como o funcionário mudou de setor, seu cargo foi de chefe para colaborador.\nSeu cargo pode ser atualizado novamente.");
								}
								...
								if(NovoCargo == 1) {
									...
									else if(UpdatedFuncionario.getContrato().getCargo().equals("chefe")) {
										System.out.println("Funcionário já está presente como chefe deste setor");
									}
									else {
			...
			// (4) Demitir um funcionário por meio de seu CPF:
				...
				empresa.DemitirFuncinario(FireCPF);
			}
			...
			// (6) Filtrar a lista de funcionários por setor, sexo ou estado:
				...
				if(VisualizarInput.equals("1")) {
					...
					empresa.AdvancedPrintList(3,"","",VisualizarEstadoInput);
				}
			}
